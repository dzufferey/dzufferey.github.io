...
;transition:
; S: send x to all
; R: if receive all same then vote
(declare-fun witness1 (Process) Process)
(declare-fun witness2 (Process) Process)
(assert
  (forall ((i Process))
    (or
      (and
        (elem (HO i) (witness1 i))
        (= (x1 i) (x (witness1 i)))
        (forall ((j Process))
      ...
      (and
        (elem (HO i) (witness1 i))
        (elem (HO i) (witness2 i))
        (not (= (x (witness1 i)) (x (witness2 i))))
        (not (elem voting1 i))
      )
    )
  )
...
;transition relation for round 3
(declare-fun voting_witness (Process) Process)
(declare-fun non_voting_witness (Process) Process)
(assert
  (forall ((i Process))
